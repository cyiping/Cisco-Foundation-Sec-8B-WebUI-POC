...
* **架構基礎**：基於 Llama 3.1 architecture，從零開始針對資安領域進行優化。
...
| **app3.py** | Markdown 優化版 (Kaggle T4) | **自動安裝環境**、**Markdown 格式優化**、**Gemini 風格 UI**。 | 在 Kaggle Notebook 環境中快速佈署、需要美觀且結構化的資安報告時。 |
...
#### `app3.py` 的主要改進：
*   **自動化環境配置**：內建 `install_dependencies()` 邏輯，自動檢查並安裝指定版本套件（如 `bitsandbytes>=0.43.0`），解決 Kaggle 環境常見的相容性問題。
*   **Markdown 格式優化**：透過 System Prompt 強制模型輸出結構化的 Markdown 內容（包含標題、粗體、列表），並在 Gradio 中完美渲染。
*   **Gemini 風格 UI**：加入自定義 CSS，打造類似 Gemini 的簡潔、現代化互動介面，提升閱讀體驗。
*   **Kaggle Secrets 整合**：支援從 Kaggle Secrets 自動讀取 `HF_TOKEN`，實現無縫登入。
...
├── app3.py               # Markdown 優化版 (含自動安裝與 Gemini UI)
